codeunit 50101 "SQmodFunction S365"
{
    ...
    begin
        ...
        if Rec."Amend & Archived S365" then SalesHeader."No.":=IncStr(Rec."No.")
        else
        ...
        if Rec."Original Quote No. S365" <> '' then SalesHeader."Original Quote No. S365":=Rec."Original Quote No. S365"
        else
    ...
    begin
        if SalesReceivablesSetup.ArchiveandDeleteQuoteS365 then Message(Text002Msg, this.SalesQuoteNo)
        else
    ...
    begin
        SalesHdr.get(SalesHdr."Document Type"::Quote, Rec."Document No.");
        ItemCharges.SetRange("QuoteType S365", SalesHdr."Quote Type S365");
        if ItemCharges.FindSet()then repeat NewSalesLine.SetHideValidationDialog(true);
                ...
                NewSalesLine.validate("Quantity", 1); // Default quantity, editable
                NewSalesLine.validate("Unit of Measure code", ItemCharges."Charge UOM S365");
                NewSalesLine.validate("Unit Price", ItemCharges."Charge Amount S365");
                NewSalesLine.insert();
            until ItemCharges.Next() = 0;
    end;
    local procedure GetNextLineNo(var Rec: Record "Sales Line"): Integer begin
        SalesLine.Reset();
        SalesLine.SetRange("Document Type", Rec."Document Type");
        SalesLine.SetRange("Document No.", Rec."Document No.");
        if SalesLine.FindLast()then exit(SalesLine."Line No." + 10000)
        else
            exit(10000);
    end;
    ...
    begin
        ...
        if Page.RunModal(Page::ProjectTemplateS365, Jobs) = Action::LookupOK then if CopyfromJob.Get(Jobs."No.")then begin
    ...
    begin
        ...
        if WhseSetup.Get()then NextWhseReceiptNo:=NoSeries.GetNextNo(WhseSetup."Whse. Receipt Nos.")
        else
        ...
        if SalesLine.FindSet()then repeat // Create Warehouse Receipt Line
